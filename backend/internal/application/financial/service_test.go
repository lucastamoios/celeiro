...
// makes: categories, budgets, transactions, and planned entries (for income).
func pacingTestSetup(mockRepo *MockRepository, orgID int, categories []CategoryModel, budgets []CategoryBudgetModel, plannedEntries []PlannedEntryModel) {
	...
	mockRepo.On("FetchPlannedEntries", mock.Anything, mock.Anything).Return(plannedEntries, nil)
}
...
func TestGetControllableCategoryPacing_FiltersBelowOnePercentOfIncome(t *testing.T) {
	...
	categories := []CategoryModel{
		...
		{CategoryID: 20, Name: "Presentes e Doações", Icon: "🎁", IsControllable: true},
	}
	// Planned income is 6500, so the 1% threshold is 65.00.
	budgets := []CategoryBudgetModel{
		{CategoryID: 10, ControlledAmount: decimal.NewFromInt(1)},   // below threshold -> hidden
		{CategoryID: 20, ControlledAmount: decimal.NewFromInt(200)}, // above threshold -> shown
	}
	plannedEntries := []PlannedEntryModel{
		{EntryType: PlannedEntryTypeIncome, Amount: decimal.NewFromInt(6500), IsRecurrent: true},
	}
	pacingTestSetup(mockRepo, orgID, categories, budgets, plannedEntries)

	result, err := svc.GetControllableCategoryPacing(ctx, GetControllableCategoryPacingInput{
		UserID: 1, OrganizationID: orgID, Month: 6, Year: 2026,
	})

	assert.NoError(t, err)
	assert.Len(t, result.Categories, 1)
	assert.Equal(t, "Presentes e Doações", result.Categories[0].CategoryName)
}

func TestGetControllableCategoryPacing_NoPlannedIncomeShowsAll(t *testing.T) {
	mockRepo := new(MockRepository)
	svc := &service{Repository: mockRepo, system: system.NewSystem()}
	ctx := context.Background()
	orgID := 1
	...
	budgets := []CategoryBudgetModel{
		...
		{CategoryID: 20, ControlledAmount: decimal.NewFromInt(200)},
	}
	// No planned income entries: nothing to filter against, so everything shows.
	pacingTestSetup(mockRepo, orgID, categories, budgets, []PlannedEntryModel{})
...
func TestGetControllableCategoryPacing_OneTimeIncomeTargetsMonth(t *testing.T) {
	mockRepo := new(MockRepository)
	svc := &service{Repository: mockRepo, system: system.NewSystem()}
	ctx := context.Background()
	orgID := 1
	...
	budgets := []CategoryBudgetModel{
		{CategoryID: 10, ControlledAmount: decimal.NewFromInt(1)}, // 1.00, threshold is 65 -> hidden
	}
	// A one-time income entry that targets exactly this month counts toward income;
	// an entry targeting a different month must be ignored.
	otherMonth := 5
	plannedEntries := []PlannedEntryModel{
		{EntryType: PlannedEntryTypeIncome, Amount: decimal.NewFromInt(6500), IsRecurrent: false, TargetMonth: &month, TargetYear: &year},
		{EntryType: PlannedEntryTypeIncome, Amount: decimal.NewFromInt(100000), IsRecurrent: false, TargetMonth: &otherMonth, TargetYear: &year},
	}
	pacingTestSetup(mockRepo, orgID, categories, budgets, plannedEntries)

	result, err := svc.GetControllableCategoryPacing(ctx, GetControllableCategoryPacingInput{
		UserID: 1, OrganizationID: orgID, Month: month, Year: year,
	})
