...
func (s *service) ConsolidateCategoryBudget(ctx context.Context, params ConsolidateCategoryBudgetInput) (MonthlySnapshot, error) {
	...
	spending, err := s.Repository.FetchSpendingByCategory(ctx, fetchSpendingByCategoryParams{
		OrganizationID: params.OrganizationID,
		Month:          budget.Month,
		Year:           budget.Year,
	})
...
func (s *service) GetPlannedEntriesForMonth(ctx context.Context, params GetPlannedEntriesForMonthInput) ([]PlannedEntryWithStatus, error) {
	...
	// Fetch non-recurrent entries with savings_goal_id for this month
	// (these are auto-generated by savings goals and need to appear in the budget)
	isNotRecurrent := false
	allNonRecurrent, err := s.Repository.FetchPlannedEntries(ctx, fetchPlannedEntriesParams{
		UserID:         params.UserID,
		OrganizationID: params.OrganizationID,
	...
	for _, entry := range allNonRecurrent {
		if entry.SavingsGoalID != nil && int(entry.CreatedAt.Month()) == params.Month && entry.CreatedAt.Year() == params.Year {
