...
## Summary
Service layer for managing planned future transactions (recurrent and one-time) that feed into calculated budgets.
...
- Depends on: category-budgets-data (requires planned_entries table)
- Related to: lazy-consolidation (instances created during month initialization)
