          ...
          >
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {/* Recurrent indicator */}
                    {Pattern.IsRecurrent && (
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-wheat-100 text-wheat-800">
                        🔁 Recorrente
                      </span>
                    )}
                  </div>
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="ml-3 inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-gradient-to-r from-wheat-500 to-wheat-600 hover:from-wheat-600 hover:to-wheat-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-wheat-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
            ...
            {isExpanded && (
              <div className="px-4 pb-4 pt-2 border-t border-stone-100">
                <h4 className="text-xs font-semibold text-stone-700 mb-3">
...
function ScoreBar({ label, score, weight, description }: ScoreBarProps) {
  ...
  const getColor = () => {
    if (score >= 0.8) return 'bg-sage-500';
    if (score >= 0.6) return 'bg-terra-500';
    if (score >= 0.4) return 'bg-terra-400';
    return 'bg-rust-500';
  };
